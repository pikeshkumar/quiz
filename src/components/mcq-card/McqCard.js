    ...
    <div>
      {
        <div className="mcq-dashboard">
          {currentQuestion < props.qSize ? (
            <div>
              <div className="mcq-title">{props.title}</div>
              <div className="mcq-question-layout">
                ...
                <FormControl>
                  ...
                  >
                    {props.data[currentQuestion].option.map((optVal, index) => {
                      return (
                        <FormControlLabel
                          key={index}
                          value={optVal.slice(0, -1)}
                          control={
                            // <Radio
                            // 	onChange={() =>
                            // 		getAnswer(
                            ...
                            props.questionType === "single" ? (
                              <Radio
                                checked={selectedAnswer === optVal}
                                onChange={() => getAnswer(optVal)}
                              />
                            ) : props.questionType !== "single" ? (
                              <Checkbox onChange={() => getAnswer(optVal)} />
                            ) : (
                              <Radio onChange={() => getAnswer(optVal)} />
                            )
                          }
                          label={optVal.slice(0, -1)}
                        />
                      );
                    })}
                  </RadioGroup>
              ...
              <div className="mcq-next-q">
                <div className="mcq-total-q">
                  <b>Question {currentQuestion + 1}</b>/{props.qSize}
                </div>
            ...
            <div
              hidden={currentQuestion == props.qSize ? false : true}
              className="result-style"
            >
              <div>
                <img src={Celebration} />
                <div className="congratulation-style">Congratulations</div>
              </div>
              <div className="score-style">
                You've scored {score} out of {props.qSize}
              </div>
            </div>
          )}
        </div>
